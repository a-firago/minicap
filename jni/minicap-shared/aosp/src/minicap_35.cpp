...
private:
  ...
  createVirtualDisplay() {
    ...
    // Android 15: createDisplay(String8, bool) was replaced by
    // createVirtualDisplay(std::string, bool, ...).
  ...
  destroyVirtualDisplay() {
    ...
    // Android 15 (SDK 35) replaced destroyDisplay() with destroyVirtualDisplay().
...
minicap_try_get_display_info(int32_t displayId, Minicap::DisplayInfo* info) {
  ...
  // Android 15: displays are addressed by PhysicalDisplayId, and
  // getInternalDisplayToken() was removed. Treat the incoming displayId as an
  // index into the list of physical displays (0 == primary), matching the
  // previous internal-display fallback behaviour.
  const std::vector<android::PhysicalDisplayId> displayIds =
    android::SurfaceComposerClient::getPhysicalDisplayIds();
  if (displayIds.empty()) {
    MCERROR("SurfaceComposerClient::getPhysicalDisplayIds() returned no displays\n");
    return android::NAME_NOT_FOUND;
  }

  size_t index = (displayId >= 0 && static_cast<size_t>(displayId) < displayIds.size())
    ? static_cast<size_t>(displayId) : 0;
  const android::PhysicalDisplayId physicalId = displayIds[index];

  android::sp<android::IBinder> dpy =
    android::SurfaceComposerClient::getPhysicalDisplayToken(physicalId);
  if (!dpy) {
    MCERROR("could not get display token for id: %d\n", displayId);
    return android::NAME_NOT_FOUND;
  }

  android::ui::StaticDisplayInfo dinfo;
  err = android::SurfaceComposerClient::getStaticDisplayInfo(physicalId.value, &dinfo);
